...
import 'package:otus_food_app/feature/auth/data/mock_auth_repository.dart';
import 'package:otus_food_app/feature/auth/domain/auth_state/auth_cubit.dart';
import 'package:otus_food_app/feature/recipe_list/data/mock_recipe_list_repository.dart';
...
void main() {
  ...
  testWidgets('Отображение всех виджетов на странице описания рецепта',
      (WidgetTester widgetTester) async {
    TestWidgetsFlutterBinding.ensureInitialized();

    final widget = RecipeDetail(
      recipe: mockRecipe,
      key: key,
    );
    ...
    await mockHydratedStorage(() async {
      await widgetTester.pumpWidget(MultiBlocProvider(providers: [
        BlocProvider<AuthCubit>(
          create: (context) => AuthCubit(MockAuthRepository()),
        ),
        BlocProvider<RecipeListCubit>(
          create: (context) => RecipeListCubit(MockRecipeListRepository()),
        ),
      ], child: wrap(widget)));
    });
    await widgetTester.pumpAndSettle();

    await expectLater(find.byKey(key), matchesGoldenFile('recipe_detail.png'));
  });
}
